#!/usr/bin/env node
/**
 * ResiResi × APIblaze — guided lab, TERMINAL driver (./launch_terminal_only.sh / .\launch_terminal_only.cmd).
 *
...
runLab(io, { panes: false })
  ...
  .catch((e) => {
    console.error(s.red("\n  ✗ " + (e?.message || e)));
    console.error(s.dim("  Fix the issue above and re-run  ./launch_terminal_only.sh  (or  .\\launch_terminal_only.cmd ) — it resumes with the same proxy."));
    rl.close();
    process.exit(1);
  });
